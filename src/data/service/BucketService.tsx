...
export const BucketService = {
  ...
  async createBucket(newBucketData: Entities.Bucket) {
    try {
      const response = await axios.post("http://localhost:8080/create_bucket", {
  ...
  async updateBucket(updateBucketData: Entities.Bucket) {
    try {
      const response = await axios.put("http://localhost:8080/update_bucket", {
      ...
      console.error("Erro ao atualizar um bucket:", error);
      throw error;
    }
  },
  ...
  getBucketsSmall() {
    return fetch("/demo/data/buckets-small.json", {
  ...
  getBucketsss() {
    return fetch("/demo/data/buckets.json", {
      headers: { "Cache-Control": "no-cache" },
    })
      .then((res) => res.json())
      .then((d) => d.data as Demo.Bucket[]);
